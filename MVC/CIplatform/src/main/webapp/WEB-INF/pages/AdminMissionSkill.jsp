...
<body>
            ...
            <div class="col py-3">
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <button class="addbutton">
	                    <a class="d-flex align-items-center btn" href="addNewSkillPage?uid=${user.user_id}">
                ...
                <table id="example" class="display mt-4" style="width:100%">
                    <thead>
                        <tr>
                            <th>Mission Skill id</th>
                            <th>Mission Skill title</th>
                            <th>Status</th>
                            <th>Action</th>
                        </tr>
                    </thead>
    ...
    <script>
        ...
        function loadAllSkillsForAdmin(){
        	$.ajax({
				url : "loadAllSkillsForAdmin",
				dataType : 'json',
				type : "GET",
				success : function(response) {
					console.log(response);
					setAllSkillsForAdmin(response);
				}
			});
        }
        function setAllSkillsForAdmin(skill){
        	table
            .clear()
            .draw();
        	let GEB="";
        	...
        	for(var i in skill){
        		GEB=`<div class="d-flex gap-2">
        				<a type="submit" class="d-flex btn" href="editSkillPage?sid=`+skill[i].skill_id+`&auid=${user.user_id}"><img src="images/editing.png" alt=""></a>
   	            	 <button onclick="deleteSkill(`+skill[i].skill_id+`)"><img src="images/delete.png" alt="" ></button></div>`;
        		if(skill[i].status=="ACTIVE"){
   	            	status=`<div style="color:green">`+skill[i].status+`</div>`;
   	            }
   	            else{
   	            	status=`<div style="color:red">`+skill[i].status+`</div>`;
   	            }
        		table.row.add([
        			skill[i].skill_id,
        			skill[i].skill_name,
